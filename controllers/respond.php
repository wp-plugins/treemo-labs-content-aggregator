...
class JSON_API_Respond_Controller {
  ...
  function submit_comment() {
    ...
    if (empty($_REQUEST['post_id'])) {
      $treemo_json_api->error("No post specified. Include 'post_id' var in your request.");
    } else if (empty($_REQUEST['name']) ||
               empty($_REQUEST['email']) ||
               empty($_REQUEST['content'])) {
      $treemo_json_api->error("Please include all required arguments (name, email, content).");
